...
import com.application.facedec.dto.Attendance.*;
...
import com.application.facedec.service.Attendance.AttendanceService;
...
public class AttendanceController {
    ...
    public ResponseEntity<?> inLogAttendance(@RequestBody InLogRequest inLogRequest){
        ...
        if (currentUser == null) {
            return geh.handleError("User not authenticated.",false, HttpStatus.UNAUTHORIZED);
        }

        double latitude = inLogRequest.getLatitude();
        double longitude = inLogRequest.getLongitude();
        long fcss = inLogRequest.getFcss();
        boolean isFaceMatched = inLogRequest.getIsFaceMatched() == 1;
        ...
        try {
            Attendance markedAttendance = attendanceService.markInTime(currentUser, latitude, longitude, fcss, isFaceMatched);

            if (markedAttendance != null) {
                Map<String, Object> response = getStringObjectMap(currentUser, markedAttendance);
                return ResponseEntity.status(HttpStatus.CREATED).body(response);
            } else {
                return geh.handleError("Failed to log in-time. It might already be marked for today.", false, HttpStatus.CONFLICT);
            }
        } catch (Exception e) {
            e.printStackTrace();
            return geh.handleError(STR."Error logging in-time attendance: \{e.getMessage()}", false, HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }

    private static Map<String, Object> getStringObjectMap(Employee currentUser, Attendance markedAttendance) {
    ...
    public ResponseEntity<?> outLogAttendance(@RequestBody OutLogRequest outLogRequest) {
        ...
        try {
            ...
            if (markedAttendance != null) {
                OutLogResponse response = new OutLogResponse();
                response.setMessage("Out-time attendance logged successfully.");
                response.setEmployeeId(currentUser.getId());
                response.setAttendanceId(String.valueOf(markedAttendance.getId()));
                response.setOutTime(markedAttendance.getOutTime().toString());
                ...
                if (!hasInTime) {
                    return geh.handleError("No 'in' time record found for today. Cannot mark 'out' time.", false, HttpStatus.NOT_FOUND);
                } else if (hasOutTime) {
                    return geh.handleError("Out-time already marked for today.", false, HttpStatus.CONFLICT);
                } else {
                    return geh.handleError("Failed to log out-time attendance.", false, HttpStatus.INTERNAL_SERVER_ERROR);
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
            return geh.handleError(STR."Error logging out-time attendance: \{e.getMessage()}", false, HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    public ResponseEntity<?> markHolidayStatus(@RequestParam("date") String dateString,
        ...
        try {
            ...
            return geh.handleError("Invalid date format (use YYYY-MM-DD) or invalid holiday status.", false, HttpStatus.BAD_REQUEST);
        }
        ...
        try {
            ...
            if (updatedAttendance != null) {
                ...
                return geh.handleError("Failed to mark holiday status.", false, HttpStatus.INTERNAL_SERVER_ERROR);
            }
        } catch (Exception e) {
            e.printStackTrace();
            return geh.handleError(STR."Error marking holiday status: \{e.getMessage()}", false, HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    @GetMapping("/todayLog")
    public ResponseEntity<?> todayLog() {
        Employee currentUser = securityUtils.getAuthenticatedUser();
        ...
        if (currentUser == null) {
            return ResponseEntity
                    ...
                    .body("User is not authenticated or session expired.");
        }

        try {
            TodayLogResponse todayLog = attendanceService.getTodayLog(currentUser);

            // 2. Return the DTO directly with an OK (200) status
            // A DTO is usually sufficient for a simple response, no need for a Map
            return ResponseEntity.ok(todayLog);
        } catch (Exception e) {
            // 3. General error handling (e.g., database connection issues, etc.)
            // Log the error (e.g., logger.error("Error fetching log", e);)
            return ResponseEntity
                    .status(HttpStatus.INTERNAL_SERVER_ERROR) // 500
                    .body("An unexpected error occurred while fetching today's log.");
        }
    }

    @GetMapping("/monthlyLog")
    public ResponseEntity<?> monthlyLog(
        ...
        // Basic validation for month/year consistency
        if ((month != null && (month < 1 || month > 12)) || (year != null && year < 2000)) {
            return ResponseEntity
                    .status(HttpStatus.BAD_REQUEST)
                    .body("Invalid month (1-12) or year provided.");
        }

        try {
            List<DailyLog> monthlyLog = attendanceService.getMonthlyLog(currentUser, month, year);

            // Returns a list of daily logs for the month
            return ResponseEntity.ok(monthlyLog); // Status 200

        } catch (Exception e) {
            // Log the exception details here for debugging (e.g., logger.error("...", e))
            return ResponseEntity
                    .status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body("An unexpected error occurred while fetching the monthly log.");
        }
    }

//    Types: date A -> date B || Month X || Year X
//    Allowences (Import only Admins , Export Everyone)
...
//    @GetMapping("/hoursThisWeek")
//    public ResponseEntity<?> hoursThisWeek() {}

//    @GetMapping("/LeavesRemaining")
//    public ResponseEntity<?> leavesRemaining() {}
